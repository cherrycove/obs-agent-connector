...
| Turn outcome | `TurnCompleted.stop_reason`, `agent_result`, and optional aggregate usage | Turn | Authoritative terminal evidence; aggregate usage remains exact on the root and can be apportioned across an exact matching set of calls with explicit estimated provenance |
...
| Agent Monitoring reads token fields from `llm` spans | Multi-call traces need LLM-level values even though Grok exposes only an exact turn aggregate | Preserve the exact root aggregate and use conservation-based estimated allocation across the validated call set | Prefer native per-response usage when Grok exposes it |
