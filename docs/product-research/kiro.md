...
The modern `promptTurnSummaries` values are billing credits, not token counts, and are never mapped to token usage. Legacy storage exposes aggregate turn usage rather than verified per-LLM usage. For legacy multi-call tool chains the adapter keeps usage on the root Turn and does not fabricate per-call token allocation.
