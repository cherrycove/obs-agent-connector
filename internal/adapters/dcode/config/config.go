...
func Resolve(options ResolveOptions) Config {
	...
	merged := map[string]any{
		...
		"resourceAttributes": map[string]any{
			"service.name": "gtrace-dcode", "telemetry.sdk.name": "gtrace",
			"telemetry.sdk.language": "go", "agent_runtime": "dcode",
		},
	}
	for _, source := range []map[string]any{
		readJSON(filepath.Join(home, ".deepagents", "gtrace.json")),
		readJSON(filepath.Join(cwd, ".deepagents", "gtrace.json")),
		readJSON(agentfiles.ConfigPath(home, "dcode")),
	...
	if stateDir == "" {
		stateDir = filepath.Join(agentfiles.Directory(home, "dcode"), "state")
	}
	return Config{
		Enabled: enabled,
		Transport: transport.Config{
		...
		LogFile:            agentfiles.HookLogPath(home, "dcode"),
		StateDir:           stateDir,
	}
}
...
func environmentConfig(env map[string]string) map[string]any {
	return clean(map[string]any{
		"enabled":            firstEnv(env, "DCODE_OTEL_ENABLED", "DEEPAGENTS_OTEL_ENABLED", "TRACE_TO_GTRACE"),
		"endpoint":           firstEnv(env, "OTEL_EXPORTER_OTLP_ENDPOINT", "DCODE_OTEL_ENDPOINT", "DEEPAGENTS_OTEL_ENDPOINT", "GTRACE_ENDPOINT"),
		"otel_traces_url":    firstEnv(env, "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT", "DCODE_OTEL_TRACES_ENDPOINT"),
		"otel_metrics_url":   firstEnv(env, "OTEL_EXPORTER_OTLP_METRICS_ENDPOINT", "DCODE_OTEL_METRICS_ENDPOINT"),
		"tracePath":          firstEnv(env, "DCODE_OTEL_TRACE_PATH", "GTRACE_TRACE_PATH"),
		"metricsPath":        firstEnv(env, "DCODE_OTEL_METRICS_PATH", "GTRACE_METRICS_PATH"),
		"headers":            parseObject(firstEnv(env, "OTEL_EXPORTER_OTLP_HEADERS", "DCODE_OTEL_HEADERS")),
		"resourceAttributes": parseObject(firstEnv(env, "OTEL_RESOURCE_ATTRIBUTES", "DCODE_OTEL_RESOURCE_ATTRIBUTES")),
		"captureContent":     firstEnv(env, "DCODE_OTEL_CAPTURE_CONTENT", "GTRACE_CAPTURE_CONTENT"),
		"maxChars":           firstEnv(env, "DCODE_OTEL_MAX_CHARS", "GTRACE_MAX_CHARS"),
		"timeoutMs":          firstEnv(env, "DCODE_OTEL_TIMEOUT_MS", "GTRACE_TIMEOUT_MS"),
		"debug":              firstEnv(env, "DCODE_OTEL_DEBUG", "GTRACE_DEBUG"),
		"stateDir":           firstEnv(env, "DCODE_OTEL_STATE_DIR"),
	})
}
